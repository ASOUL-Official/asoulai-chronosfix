...
class PipelineTests(unittest.TestCase):
    def test_pipeline_proves_cause_and_selects_patch(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(state.approval_record["state_revision"], state.state_revision)
            ...
            self.assertEqual(state.orchestration_status, "COMPLETED")
            self.assertGreaterEqual(len(state.task_graph), 6)
            coordination_types = {item["event_type"] for item in state.orchestration_events}
            self.assertIn("task_reassigned", coordination_types)
            self.assertIn("evidence_deduplicated", coordination_types)
            self.assertIn("approval_invalidated", coordination_types)
            self.assertIn("human_resume", coordination_types)
            self.assertIn("task_deduplicated", coordination_types)
            self.assertTrue(state.discovered_skills)
            self.assertTrue((Path(temp_dir) / "trace.jsonl").exists())
            self.assertTrue((Path(temp_dir) / "coordination.json").exists())
