...
仓库现已提供受控 `scripts/github_draft_pr.py` 适配器：它使用 GitHub App/PAT 最小权限创建证据分支和 Draft PR，并保存 commit、PR URL、request ID 和外部写入审计；它仍不修改业务源码、不创建 Check Run、不合并 PR。只有实际执行该脚本并核验在线链接后，才能把对应运行标为 external execution evidence。
...
- 61 项测试、严格 JSON/JSONL、公开 Draft 2020-12 Schema；
